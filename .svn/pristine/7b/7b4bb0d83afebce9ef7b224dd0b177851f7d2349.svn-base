			...
			for (var i = 0; i < ArrParameters.length; i++) {
				...
				if (ArrParameters[i].name == 'Location') {
					if (undefined !== ArrParameters[i].value && !$.isEmptyObject(ArrParameters[i].value)){					
						var locLatLngVal = $.parseJSON(stripSlashes(ArrParameters[i].value));
				...
				if(isInitialData){
					VarLatestPointsList += '<div class="form-row"><label><span>'+ArrParameters[i].name+'</span><input type="text" readonly  name="'+ArrParameters[i].name+'"  id="'+ArrParameters[i].name+'" value="'+ pointDataValue +'"><p id="device-unit">'+FnCheckForUnitless(ArrParameters[i].unit)+'</p></label></div>';
				}else{
					$('#'+ArrParameters[i].name).val(pointDataValue);
				}
			}
